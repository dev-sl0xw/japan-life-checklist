...
> 처음이라면 페이지 우측 상단 **「❓ 사용법」** 버튼(또는 [`guide.html`](https://dev-sl0xw.github.io/japan-life-checklist/guide.html))에서 3분 시작 가이드를 한국어/日本語로 확인하세요.

![도메인 뷰](docs/img/screenshot-domain.png)

## 특징

- **사용법 페이지**: 상단 「❓ 사용법」 버튼으로 별도 가이드 페이지(`guide.html`)에서 단계별 시작법·위험도 색상 의미를 한·일로 확인
- **조건부 공개**: 이사/이직/공백기/자동차/면허/자녀·배우자/비자변경/**부업·프리랜서** 등 플래그로 내 상황에 맞는 항목만 표시. 일부 항목(영구 귀국 등)은 「준비중」으로 표시되어 추후 제공 예정
- **부업·프리랜서 세무**: 부업 소득 플래그를 켜면 확정신고(20만 엔)·주민세 보통징수·개업신고·청색신고·장부보존·인보이스 등 国税庁 공식 출처 기반 항목 표시
- **나만의 D-Day**: 「이사 예정일 / 퇴직일」을 입력하면 각 항목이 **실제 달력 날짜 + 남은 일수**로 계산되고, 임박할수록 색이 강해집니다(D-5 이내 빨강 등). 미입력 시 상대 라벨 유지 — 다른 사람도 자기 날짜로 그대로 사용 가능
- **거주 지역 선택**: 도도부현(47) + **도쿄 23구**(2단 선택) 선택 시 대형쓰레기·役所 등 지역별 공식 링크가 반영됩니다. 도쿄 23구는 각 구의 粗大ごみ受付セン터 신청 페이지로 직접 연결, 미등록 지역은 전국 공통 안내로 폴백
...
- **한국어 / 日本語 전체 전환**: 헤더의 한·일 토글로 UI·항목·설명까지 페이지 전체 언어를 전환(기본 한국어, 선택 언어는 저장됨). 선택 언어가 주, 다른 언어가 부제로 병기
- **양방향 검색**: 한국어로 검색해도 일본어 원문(`任意継続` 등)에 매치
...
**v1**: 이사 + 이직 + 그에 직결되는 외국인/한국인 필수 행정(재류카드·마이넘버·사회보험·연금·주민세·퇴직문서·은행·통신·NHK·자동차) + **부업·프리랜서 세무**.

**v1.5+ (예정)**: 한국 세무·외환·영사관·병역·해외재산조서, 영구귀국, 자녀학교 상세, 펫, 자가매각, **지역팩 확장**(현재 도쿄 23구 대형쓰레기까지 — 도쿄 多摩·기타 도부현 구/시 및 役所 등 다른 지역항목으로 확대), PWA.
